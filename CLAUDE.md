...
- **내신 참여표**(`ExamForm`/`ExamBoard`): 구글시트로 쓰던 «클라이매쓰 내신관리»를 옮긴 것.
  ...
  - **등원 회차**(`attendCount`)를 그 달 기준으로 센다. **수강료에 쓰는 숫자다.**
    `수업일 − 내신기간에 걸치는 날 + 직보`. 직보는 수업 요일이 아니어도, 내신기간
    안이어도 센다(학생이 오니까). 이미 센 날이면 중복으로 안 센다.
  ...
  - 날짜 띠는 **고른 달 1일~말일**. 달 선택에 **앞으로 4개월**이 들어간다(`examMonths`) —
    9월 시험 준비를 8월에 하므로 지난 달만 주면 정작 필요한 달이 없다.
