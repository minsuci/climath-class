...
### 0-2. 학교 일정 «찾아오기» 서버는 팀체크로 갔다 (2026-09-08)

`api/schedule.js` 는 **팀체크의 `api/schedule.js`** 로 옮겨갔다. 여기 것은 아무도 안 부르는 사본이고,
한 회차 돌려 본 뒤 지운다. **여기를 고치지 마라. 저쪽을 고쳐라.**

옮긴 이유: 학교 일정을 채우는 일은 팀체크에서 하는데 찾아오는 코드가 여기 있어서,
가정통신문에서 수학시험 시간표를 긁는 일을 하려면 남의 앱을 고쳐야 했다.
`appConfig/neisCodes`(학교 코드 캐시)도 이제 이 DB 에서 안 쓴다 — 팀 DB 의 `dash/neisCodes` 로 갔다.
...
- **모바일 사이드바**(`.cm-side`, 860px 이하): 높이를 `100dvh`로 **직접 준다.**
  `position:fixed; top:0; bottom:0` 만으로는 상자가 안 늘어나고 내용 높이 그대로여서,
  메뉴가 길면 화면 밖으로 흘러나가기만 하고 스크롤이 안 걸린다(상자 입장에선 다 들어간 셈).
  `100vh`를 먼저 쓰고 `100dvh`로 덮는다 — 주소창이 접혔다 펴지는 아이폰 사파리 때문.
  `overscroll-behavior: contain`으로 서랍 스크롤이 뒤 페이지로 넘어가지 않게 한다.
...
### 학교 일정 — **이 앱에 없다** (2026-09-04 대시보드로 넘김)

학교·학년마다 내신 기간과 수학시험 날짜를 한 번만 적어두는 것. 여기서 시작된 문제:
학생 40명이 각자 적어 내니 같은 학교인데 값이 갈렸고, 안 낸 학생은 참여표에서 통째로 빠졌다.

> **채우는 일은 교무팀장이 대시보드(`climath-team`)에서 한다.** 선생님들은 참여표만 본다.
> 화면이 두 벌이면 어느 쪽이 맞는지 아무도 모른다 — 그래서 한 벌로 줄였다.

이 앱에 남은 것은 **읽는 쪽뿐**이다.

- `listenSchoolTerms` / `findSchoolTerm` — 학생 화면(`ExamForm`)이 자기 학교 일정으로
  빈 칸을 미리 채우고, 참여표가 «같은 학교인데 일정이 갈림»을 잡는 데 쓴다
- 서버 `api/schedule.js` 는 **그대로 둔다.** 대시보드가 그것을 중계해서 쓴다
  (나이스 → 학교 홈페이지 → 게시판 첨부문서 → AI). 로직은 여기 한 벌뿐이다

옛 화면(`SchoolTermBoard`·`SrcCell`·`runNeisPull`·`NEIS_JOB`)은 지웠다. 750줄.
...
- **나이스에서 불러오기**(`api/schedule.js`): 학교가 나이스에 올린 학사일정에서
  내신 기간을 찾아 빈 칸을 채운다. 이미 적힌 칸은 **덮지 않고** 다른 것만 보여준다.
  - 짧은 이름을 정식 이름으로 편다(숙명여고 → 숙명여자고등학교). `X대부고` 넷은 별칭표.
    한 번 찾은 학교 코드는 `appConfig/neisCodes`에 적어둔다.
  - ⚠ **키가 없으면 한 번에 5건, 그리고 `pIndex`가 안 먹는다** — 2페이지를 불러도
    1페이지와 같은 5건이 온다. 그래서 날짜 창을 반씩 잘라 5건 이하가 될 때까지 좁힌다.
    요청이 늘어나므로 **학교 하나에 요청 하나**로 나눠 부른다(함수 실행 시간 때문).
    `NEIS_API_KEY`(무료, open.neis.go.kr)를 Vercel에 넣으면 요청 한 번으로 끝난다.
  - **수학 시험 날짜는 못 가져온다.** 학사일정에는 과목별 시간표가 없다.
  - 시험 이름은 학교마다 다르다. 경기도 쪽은 "1차/2차 지필평가"라고 쓴다 —
    `kind`를 글자 그대로 맞추면(`indexOf("중간")`) 그런 학교가 통째로 빠진다(`KIND_RE`).
  - **나이스에 시험이 없으면 학교 홈페이지를 본다**(`homepageExams`). 서울 교육청
    웹호스팅 학교는 생김새가 같아서, 메뉴에서 "학사일정/월간일정/학교일정"을 찾아
    `viewType=list`로 POST 하면 표가 그대로 온다. 칸 사이가 `| |` 처럼 여러 개로 나온다.
    게시판에 PDF·한글·이미지로 올리는 학교는 여기서 못 잡는다 — 형식이 제각각이라 일반화가 안 된다.
  - **3단계: 게시판에 붙은 문서**(`boardDocText`). 서울 CMS는 게시판도 생김새가 같다:
    `bbsId` → `selectBoardListAjax.do`(글 목록) → `selectBoardDetailAjax.do`(첨부 `atchFileId`)
    → 문서뷰어 `job` → `key` → `thumbnailxml/<key>/<쪽>` 이 **변환된 글자층**을 준다.
    PDF를 직접 뜯지 않아도 글자가 나온다. ⚠ 목록·상세 AJAX는 **세션 쿠키가 있어야** 내용을 준다.
  - 그 글자에서 날짜를 뽑는 것만 AI(`examFromDoc`, GEMINI_API_KEY)에게 맡긴다.
    학사력은 학교마다 표 모양이 완전히 달라 규칙으로 짜면 학교마다 깨진다.
    **지어낼 수 없게 검증한다**: 답한 날짜의 숫자가 원문에서 실제로 그 시험 낱말 바로 앞에
    붙어 있어야 통과(`dayHasExam`). 검증 구간은 학기로 좁힌다(`semesterSlice`) —
    안 그러면 1학기의 "27중간고사"가 2학기 10월 7일을 통과시킨다. 시험 종류도 구분한다 —
    "27기말고사"의 "7기말"이 중간고사 검증을 통과하면 안 된다.
    이렇게 읽은 것은 화면에 "첨부문서에서 읽었어요 — 확인해주세요"로 표시한다.
  - **4단계: 메뉴 본문에 붙여둔 그림**(`menuImages`·`examFromImage`). 게시판 글이 아니라
    메뉴 본문(`subMenu.do`)에, 그것도 표가 아니라 **편집기로 붙여넣은 PNG**로 올리는 학교가 있다
    (서운중). HTML 에 "중간고사"가 한 글자도 없어서 위 세 갈래가 전부 빈손으로 돌아온다 —
    그걸 "일정을 안 올린다"로 보고했었다.
    ⚠ 그림은 **대조할 원문이 없다.** 그래서 `dayHasExam` 대신 **요일로 검증한다**(`verifyImagePick`) —
    표는 요일이 열이라, AI가 "21일·월 칸"이라 읽었으면 그날이 진짜 월요일이어야 한다.
    줄을 밀려 읽거나 해를 잘못 잡으면 어긋난다. 주말·열흘 초과·없는 날짜·시험 아닌 글자도 막는다.
    그래도 사람이 봐야 하므로 `via:"image"` 로 알리고 근거 링크는 그 메뉴 페이지를 가리킨다.
    ⚠ 이 파일의 `ymd()` 는 하이픈만 지운다 — `2026-02-30` 이 그냥 통과하므로 `realDate()` 로 따로 본다.
    시험: `node tools/test-schedimg.mjs` (18건)
  - 그래도 안 되는 학교는 **붙여넣기**로 넣는다: `학교 학년 시작 끝 [수학시험]` 한 줄에 하나.
    학년에 `전체`를 쓰면 고1·고2·고3에 같이 들어간다. 이건 이미 적힌 칸도 덮어쓴다.
  - 나이스에 일정을 아예 안 올리는 학교(예: 경기고)와 일정은 있는데 시험만 없는 학교를
    구분해 알려준다(`hasAny`).
  - **이미 채운 학교는 안 부른다**(`pullTargets`). 학교 하나에 몇 초씩 걸려서, 다 부르면
    이미 끝난 일에 시간을 다 쓴다. "이미 채운 것도 다시 확인"을 켜면 전부 부른다.
  - 작업 상태는 **모듈에 둔다**(`NEIS_JOB`). 화면에 매어두면 다른 메뉴를 누르는 순간
    하던 것이 통째로 날아간다. 화면은 `neisSub`으로 구독만 하고, 나갔다 들어와도 이어 보인다.
- 날짜 칸에는 **지우개(×)를 붙인다**(`.cm-datewrap` / `.cm-x`). `<input type="date">`는
  휴대폰에서 값을 비울 방법이 없다 — 달력 휠은 늘 어떤 날짜를 고르게 되어 있어서
  잘못 넣으면 되돌리지 못한다. 참여표의 복귀 칸도 같다.

### 수업 내용 자동 채우기 (2026-09-05)

코멘트를 매번 못 쓰니 «그날 무엇을 했는지» 라도 남긴다. 반에 유튜브 재생목록을 걸어두면
자동으로 채워지고, 보고서에서 **학생 코멘트가 없는 날**에 그것이 실린다.

    1층  영상 제목      매일 23시   버셀 크론 (api/cron/lessons.js)
    2층  자막 → 개념어  매일 1시    이 PC (tools/lesson-captions.cmd) + 서버 요약

- 저장 자리는 `classes/{cid}/days/{date}.lesson` 이다. 새 컬렉션을 만들면 보안 규칙을
  콘솔에 다시 붙여넣어야 하고, 안 붙여넣으면 **조용히 아무것도 안 쌓인다.**
  `days` 는 이미 반 사람이 읽고 선생님이 쓴다
- 크론은 «오늘 것» 이 아니라 **최근 7일 중 빈 날**을 메운다. 무료 요금제는 ±59분이라
  시각이 밀린다. `?days=` 로 넓혀 부르면 처음 걸 때 지난 것도 채운다
- `source` 로 누가 썼는지 가른다 — `title`(크론) · `caption`(PC 도구) · 그 밖(사람).
  **크론은 title 만 덮는다**
- 자막 요약 프롬프트에 «숫자를 쓰지 마라» 를 못박았다 —
  자동 자막이 분수·문항번호를 틀리게 옮긴다. 교재·범위는 제목에서 온 것을 괄호로 붙인다
- PC 도구는 `team/tools.lessonKey` 로만 연다. AI 키도 파이어베이스 열쇠도 PC 에 두지 않는다

### 참여표 (`ExamBoard`)

**한꺼번에 채우는 것은 이 앱에 없다** — 대시보드에서 한다(2026-09-04).
«안 낸 학생» 상자에 누가 채우는지 한 줄 밝혀 두었다. 안 밝히면 선생님이 없는 화면을
찾아 헤매거나 저절로 채워지길 기다린다.

이 앱에서 하는 것은 **학생별로 대신 넣기·고치기·복귀일 일괄 지정·동명이인 잇기**다.
