...
- 캐주얼한 톤, 빠른 실행 선호
- **작업이 끝나면 대화 내용을 md 파일로 정리해 줄 것.** 저장 위치는
  바탕화면의 `민수의 뇌` 폴더(Obsidian 등 로컬). 원격 세션에서는 직접 넣을 수
  없으므로 md 파일로 만들어 전달하면 본인이 옮긴다.
  정리 범위는 **결정과 그 이유 중심** — 무엇을 왜 그렇게 정했는지, 폐기한 대안은
  무엇이고 왜 버렸는지, 겪은 함정은 무엇인지. 주고받은 대화를 그대로 옮기지 말 것.
