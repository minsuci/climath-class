...
- **학생 명단**(`StudentDirectory`, 전역 메뉴 `roster`): 반이 아니라 **사람**을 관리한다.
  `students/{pid}` = 사람 하나. 이름 · 학교 · 학년 · **담임(tid)**.
  반 명단(`roster`) 항목이 `pid`로 그걸 가리키고, **학교·학년은 roster에도 함께 적어둔다(미러)** —
  학생 화면은 반 문서만 읽을 수 있어서 그래야 학생도 자기 학교·학년을 본다.
  사람 정보를 고치면 `syncStudentToRosters()`가 그 사람이 속한 모든 반의 미러를 갱신한다.

  > [!warning] 이름으로 잇지 말 것
  > `autoLink`는 **이름이 하나뿐일 때만** 잇는다. 겹치면 건너뛰고 사람이 고르게 한다.
  > 지금까지 반을 가로질러 학생을 잇는 유일한 수단이 이름이어서 동명이인이 섞였다.
  > `pid`가 생기면서 그 문제를 풀 토대가 됐다(로그인·`userPins`는 아직 이름 기준).

  - 내신 참여표는 이 값을 **기본값으로 자동으로 쓴다.** 학생·선생님 양쪽 폼 모두.
  - **반 배정을 여기서 바로 한다**(`assignToClass`/`unassignFromClass`). 학생 편집기의
    반 칩을 누르면 넣고 뺀다. 개별진도반에 넣으면 등원요일·교재는 반 설정에서
    따로 정하라고 안내한다. 빼도 출석·과제 기록은 남는다.
  - 목록은 **엑셀 같은 표**다(`StudentRow`, `.cm-sheet`). 칸을 바로 고치면 저장된다 —
    고르는 칸(학교·학년·담임)은 즉시, 이름은 칸을 벗어날 때. 저장되면 칸이 잠깐 초록으로.
    맨 아랫줄에 이름을 치면 새 학생이 만들어진다. `반` 칸을 누르면 배정 줄이 펼쳐진다.
  - 열 폭은 `table-layout: fixed`로 못 박는다. auto로 두면 학교 select의 가장 긴 항목이
    열을 밀어 이름 칸까지 넓어졌다. 남는 자리는 맨 오른쪽 빈 열이 먹는다.
  - 머리글을 누르면 오름 → 내림 → 원래 순서로 돈다(`sort` 상태). 빈 칸은 방향과 상관없이
    늘 아래. 학년은 글자 순이 아니라 `GRADES` 순서(중3<고1<고2<고3)로 센다.
  - 학교는 **치는 칸**이다(datalist `cm-school-list`). 칸을 벗어날 때 `matchSchool()`이
    아는 이름으로 맞춘다 — 숙명·숙명여자고등학교 → 숙명여고. 후보가 둘 이상이면
    (경기 → 경기고/경기여고) 건드리지 않는다. 학생 화면은 그대로 고르는 방식.
  - "목록에 없어요"(주황 테두리) 판정은 `curated`(KNOWN_SCHOOLS + appConfig/schools)만 본다.
    다른 학생이 쓰는 학교까지 포함한 `schoolOpts`로 판정하면 오타를 저장하는 순간
    그 오타가 "아는 학교"가 돼 표시가 사라진다.
  - 거르개가 두 줄이다. **학년**(전체 / 중3 / 고1 / 고2 / 고3 / 학년 없음)이 위,
    **반**(전체 / 반별 / 배정 없음)이 아래. 둘은 겹쳐서 걸린다.
    학생이 없는 학년도 띄워둔다 — 새 학년을 받을 자리가 보여야 한다.
  - 학년을 안 고르면 표를 **학년별로 묶어서** 보여준다(`.cm-sheet-grp` 머리줄).
    정렬은 묶음 안에서 걸린다. 반을 고르면 **그 반 명단 순서**를 묶음 안에서 지킨다.
  - `학년 없음`을 고르면 **보이는 사람을 한 학년으로 한꺼번에** 넣을 수 있다(`bulkGrade`).
    새 학년이 올라올 때 한 명씩 고르면 끝이 없다.

    > 반 명단을 다시 쓰는 함수는 전부 **최신 `cls.roster` 위에 얹어야 한다.**
  ...
  - 반 명단에 학생으로 들어간 선생님은 `선생님` 버튼으로 되돌린다. 이게 없어서
    선생님 이름이 내신 참여표에 "아직 안 낸 학생"으로 계속 떴다.
...
- **선생님 관리**(`TeacherManage`): 목록을 `listTeachers()`로 **직접 읽는다**.
  ...
  이름·역할·상태만 준다 — `classIds`가 없다. 그걸 그대로 쓰면 담당 반이 늘 빈 것으로 보여
...
### 학교 일정 (`SchoolTermBoard` · 사이드바 "학교 일정")

학교·학년마다 내신 기간과 수학시험 날짜를 **한 번만** 적어두는 곳. 여기서 시작된 문제:
학생 40명이 각자 적어 내니 같은 학교인데 값이 갈렸고(그래서 불일치 감지가 필요했다),
안 낸 학생은 참여표에서 통째로 빠졌다.

- 저장 위치는 문서 하나: `appConfig/schoolTerms`.
  열쇠는 `<회차>__<학교>__<학년>`, 값은 `{term, school, grade, start, end, math, back, updated}`.
  회차마다 값이 다르므로 열쇠에 회차를 넣는다. `appConfig/{doc}` 규칙이 이미 열려 있어
  **규칙을 다시 게시하지 않아도 된다**.
- `saveSchoolTerm()`은 **넘긴 칸만** 보낸다. `merge: true`는 map 안까지 합쳐주므로
  나머지 칸은 그대로 남는다. 통째로 보내면 화면이 들고 있던 옛 사본으로 덮게 되어
  시작 → 끝을 빠르게 이어 고칠 때 시작이 날아간다.
- 줄은 **학생이 실제로 다니는 (학교·학년)** 만 뜬다. 34개 × 4학년을 다 늘어놓으면 못 본다.
  아직 학생이 없는 학교는 맨 아랫줄에서 직접 더한다.
- 직보는 수학시험 전날로 자동, 복귀는 선택(비우면 학생별로 정한다).

### 참여표 자동 생성 (`ExamBoard`)

- **안 낸 학생 N명 채우기** — 학교·학년이 학교 일정에 걸리는 학생의 제출을 만든다.
  학교·학년은 (1) 이미 낸 값 (2) 반 명단 미러(`roster[].school/grade`) 순으로 본다.
- **학교 일정과 다른 N명 맞추기** — 이미 낸 것이 학교 일정과 어긋날 때. 자동으로 덮지 않는다:
  분반마다 진짜 다를 수 있고, 학생이 맞게 낸 걸 지우면 되돌릴 방법이 없다.
  맞출 때도 **손으로 고친 직보**(전날이 아닌 값)와 **선생님이 정한 복귀**는 건드리지 않는다.
- 학생 화면(`ExamForm`)도 학교 일정이 있으면 미리 채워준다. 이미 낸 학생은 안 건드리고,
  같은 학교로 두 번 덮지 않는다(`autoRef`) — 고쳐놓은 걸 되돌리게 된다.
- 이름·학교·학년은 **학생 명단(`students`)에서 풀어서 본다**(`whoOf`). 반 명단에도 같은 값이
  적혀 있지만 그건 학생 화면용 사본이라 안 이어진 사람에게는 없거나 낡아 있다.
  이걸 안 하면 학생 명단에서 고친 것이 이 화면에 닿지 않는다.
- "아직 안 낸 학생"도 **학년 탭을 따라간다**. 학년을 모르는 사람은 따로 모으고,
  학생 명단에 없는 사람은 `명단 없음`으로 표시한다 — 그 사람은 학교 일정으로 채울 수도 없다.
- 잘못 들어간 제출은 **지운다**(`deleteExam`). 참여표 줄과 "명단에 안 잡히는 제출" 양쪽에 있다.
  그 회차 그 학생 문서 하나만 없앤다 — 출결·과제·점수는 그대로고, 지우면 "안 낸 학생"으로 돌아간다.
